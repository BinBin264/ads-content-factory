...
- One shot contract per scene: one opening keyframe, one visible motion delta, a changed endpoint, explicit hand/prop ownership, scoped reference roles, completed dialogue, and reserved future beats.
- A compact provider prompt compiler and deterministic prompt quality gate instead of sending the whole plan to Veo.
- A required human keyframe gate: generated/uploaded keyframes must be accepted before automated clip generation. Acceptance locks prompt editing, generation, selection, and upload replacement until the operator explicitly rejects the keyframe.
- Simple clip review with `Accept Clip` and `Regenerate Clip`; clip review never rewrites Phase 2 keyframes or later prompts.

The continuity layer is adapted from the production-state ideas in the local `seedance-2.0` skill source, but it is provider-neutral and targets the current Google Veo workflow. It improves consistency through canonical references, accepted Phase 2 keyframes, per-scene re-anchoring, and scoped prompts. It does not claim that Veo can permanently guarantee a face, object, UI, or location across arbitrary generations.
...
`VIDEO_MODEL_ID` is the Phase 3 default. Automation mode can override it per scene with `veo3.1-pro`, `veo3.1-fast`, `grok-video-3`, or `grok-video-3-10s`.

Uploaded image understanding, Plan Creation, scene rewrites, keyframe prompts, and final scene prompts use Gemini exclusively. The optional ShopAIKey Gemini Image integration is isolated behind `IMAGE_PROVIDER_*` and is used only by automation-mode image generation. Image generation runs as asynchronous jobs: multiple cards can be queued together, the configured worker limit controls parallel calls, and temporary `429`, `503`, timeout, and `524` provider failures are retried serially. `IMAGE_GENERATION_MAX_RETRIES=3` means three retries after the initial request.

Automation image phases require a per-job selection from `nano-banana`, `nano-banana-2`, `nano-banana-pro`, `gpt-image-1`, `gpt-image-1.5`, `gpt-image-2`, and `gpt-image-2-all`. The backend does not choose an image model from env. Nano Banana keeps an exact `9:16` request; GPT Image uses its supported `2:3` portrait canvas and protects a centered `9:16` safe area.

There is no fallback mock LLM.

## Run Both Services

Start both services from the repository root and keep every runtime log in the single root `logs/` directory:

```powershell
.\start-dev.ps1
```

The script restarts only this workspace's processes listening on ports `8000` and `5173`. Use `-NoRestart` to leave existing listeners untouched.
...
5. For each scene, create/pick one opening keyframe image, inspect actor count, anatomy, handedness, prop ownership, and product/UI fidelity, then accept or reject it. An accepted keyframe is locked; reject it first to edit its prompt or replace the image.
6. Generate any scene whose Phase 2 keyframe has been accepted. In Manual mode, copy its prompt into Flow, Kling, or another video generation UI.
7. In Automation mode, generate one ShopAIKey `veo3.1-pro` `9:16` clip from the scene's accepted keyframe. The selected keyframe is sent as the single `metadata.images` visual anchor. `VIDEO_PROVIDER_API_KEY` is optional when it shares `IMAGE_PROVIDER_API_KEY`.
8. In Manual mode, upload or replace one clip per scene. In Automation mode, use `Accept Clip` to mark the current output complete or `Regenerate Clip` to replace only that clip. Neither action rewrites later keyframes or prompts.
